...
import { useReviewPropertyMutation } from "@/features/property/api/property.api";
...
export function ReviewPropertyDialog({
  ...
  const handleApprove = async () => {
    if (!property) return;
    await reviewProperty({ id: property.id, status: "APPROVED" }).unwrap();
    handleOpenChange(false);
  };

  const handleReject = async () => {
    if (!property) return;
    await reviewProperty({ id: property.id, status: "REJECTED", reason: reason || undefined }).unwrap();
    handleOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Duyệt khách sạn</DialogTitle>
        </DialogHeader>

        <div className="space-y-2 text-sm">
          <p>
            <span className="text-muted-foreground">Tên khách sạn: </span>
