...
export function TourScheduleDialog({
  ...
  const { data: schedule, isLoading, isError } = useListMyTourScheduleQuery(
    tour ? { tourId: tour.id, ...viewRange } : { tourId: "", startDate: "", endDate: "" },
    { skip: !open || !tour },
  );
  ...
  useEffect(() => {
    if (open) {
      reset({ ...defaultRange(), capacity: "20", price: "" });
    }
  }, [open, reset]);

  const onSubmit = async (values: FormValues) => {
    if (!tour) return;
    await setTourSchedule({
      tourId: tour.id,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-[1536px]">
        <DialogHeader>
          <DialogTitle>Ngày khởi hành — {tour?.title}</DialogTitle>
        </DialogHeader>
