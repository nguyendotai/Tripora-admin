...
import { useCreateTourMutation, useUpdateTourMutation } from "@/features/tour/api/tour.api";
...
export function TourFormDialog({
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        price: tour?.price ?? "",
        maxParticipants: tour?.maxParticipants ? String(tour.maxParticipants) : "",
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (isEdit && tour) {
      await updateTour({ id: tour.id, data: payload }).unwrap();
    } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-[864px]">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Sửa tour" : "Thêm tour"}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="durationLabel" placeholder="2 ngày 1 đêm" {...register("durationLabel")} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input id="price" inputMode="numeric" placeholder="1500000" {...register("price")} />
              {errors.price && <p className="text-xs text-destructive">{errors.price.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="maxParticipants">Số người tối đa</Label>
              <Input id="maxParticipants" inputMode="numeric" placeholder="20" {...register("maxParticipants")} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Textarea id="included" rows={3} placeholder="Xe đưa đón, HDV, bữa ăn" {...register("included")} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              rows={2}
              placeholder="Huỷ trước 3 ngày hoàn 100%"
